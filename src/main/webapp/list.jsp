...
<body class="bg-dark">
    ...
    <div class="container-md">
        <div class="p-3 mb-2 bg-dark text-white">
            <center>
            <c:if test="${empty selectedList.name}">
                <h1>Selected list does not exist</h1>
            </c:if>
            <h1>${selectedList.name}</h1>
            <ol class="list-group list-group-numbered">
            <c:forEach items="${selectedList.items}" var="item">
                <c:choose>
                    ...
                    <c:when test="${item.itemType == 'multiElementItem'}">
                        <li class="list-group-item bg-dark text-white">Multi element item : <a href="/displayMultiElemItem?itemName=${item.itemValue}" class="btn btn-outline-light">${item.itemValue}</a></li>
                    </c:when>
                    <c:otherwise>
            ...
            <a href="/addItem" class="btn btn-outline-primary">Add item to list</a>
            <a href="/editItem" class="btn btn-outline-primary">Edit item in list</a>
            <a href="/deleteItem" class="btn btn-outline-primary">Delete item in list</a>
            <br>
            <br>
